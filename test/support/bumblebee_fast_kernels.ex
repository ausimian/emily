defmodule Emily.Bumblebee.FastKernels do
  @moduledoc """
  Bumblebee shim that rewrites RMSNorm, LayerNorm, RoPE, and SDPA
  Axon layers to call `Emily.Fast.*` instead of their stock defn
  implementations. When the model is then evaluated under
  `Emily.Compiler`, those `Emily.Fast.*` calls dispatch to fused
  MLX kernels via the `:optional`-node mechanism (see
  ...
  results — so this shim doesn't gate non-Emily conformance work.

  Lives under `test/support/` because Bumblebee + Axon are `only:
  :test` deps. Graduates to `lib/` if/when Emily takes a runtime
  Bumblebee dep.
  ...
  ## Usage
      ...
      model_info = update_in(
        model_info.model,
        &Emily.Bumblebee.FastKernels.apply/1
      )

      # then proceed with Bumblebee.Text.generation/4 as usual.
  ...
  ## What's *not* rewritten
    ...
    * Attention layers with `dropout_rate > 0`. Inference path is
      ...
      training paths (when M9 grad work lands on the same graph)
      will continue using composed defn.
  """
  ...
  def fast_rope_impl(query, key, position_ids, _attention_mask, opts \\ []) do
    ...
    if freqs do
      ...
      q = Emily.Fast.rope_with_freqs(query, offset_scalar, freqs, dims: dims, traditional: false)
      ...
      q = Emily.Fast.rope(query, offset_scalar, dims: dims, traditional: false, base: base * 1.0)
  ...
  defp resolve_key_mask(%Axon.None{}), do: Nx.broadcast(Nx.tensor(1, type: {:u, 8}), {1, 1, 1, 1})
