...
- Distributed collectives over MLX's communication backends
  ...
  (RDMA-over-Thunderbolt) backends. Collectives run on a dedicated CPU
  worker, so their blocking inter-rank communication never stalls
  ordinary GPU inference on the shared worker.
